...
#define HOST "10.79.152.128" /* your zenohd IP */
#define PORT 7447
...
// Root cause of "handler never fires but the IP shows up anyway": on ESP32
// the WiFi/DHCP sequence can complete during kernel init, before main() runs
// and before we've had a chance to call net_mgmt_add_event_callback(). Zephyr
// does not replay missed events to late-registered callbacks -- if the
// NET_EVENT_IPV4_ADDR_ADD event fired before we were listening, we'll wait
// on wifi_connected_sem forever (or until timeout) even though the interface
// is already fully up. So: register the callback FIRST (already done below),
// then explicitly check "are we already connected?" rather than assuming the
// only way to find out is via the event.
